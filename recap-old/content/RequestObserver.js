...
RequestObserver.prototype = {
	...
	log(output);
	this.logRequestHeaders(channel);
    },

    // Logs interesting HTTP response headers to the Error Console
    logRequestHeaders: function(channel) {
	var headers = ["Cookie", "Referer", "Warn"];

	var output = "REQUEST Headers for " + channel.URI.asciiSpec + "\n  ";
	for (var i = 0; i < headers.length; i++) {
	    var hvalue = "";
	    try {
		hvalue = channel.getRequestHeader(headers[i]);
	    } catch(e) {
		hvalue = "<<none>>";
	    }
	...
	log("OriginalURI: " + channel.originalURI.spec);
    },
	...
	var court = null;
	try {
	    court = refhost.match(/(\w*)\.uscourts.gov/i)[1];
	} catch(e) {}
