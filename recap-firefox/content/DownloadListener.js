...
DownloadListener.prototype = {
    ...
    appendFilemetaStream: function() {
        ...
        for (var fieldname in this.filemeta) {
            if (fieldname == "name") continue;

            formData.push("--" + this.boundary);
            formData.push("Content-Disposition: form-data; name=\"" + fieldname + "\"");
            formData.push("");
    ...
    handleResponse: function(req) {
        ...
        if (jsonin && typeof(jsonin.error) == "undefined") {
            ...
            if (isPDF(this.filemeta.mimetype)) {
                ...
                if (jsonin.documents) {
                    updateMetaCache(this.metacache, jsonin.documents, jsonin.cases); // XXX
                }
