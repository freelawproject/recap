...
class PacerClient():
    ...
    def _get_request(self, request):
        ...
        if response.code != 200:
            logging.error('Error retrieving request %s' % request.get_full_url)
            return False
        return response
    ...
    def login_and_set_cookie(self, court='nysd'):
        ...
        if not self._opener:
            ...
            cookie_header = self._parse_cookie_header_from_login_response(
                response.read())
    ...
    def get_opinions_html(self, court, start_date=date.today(),
                          end_date=date.today()):
        """
        Get the written opinions report (WrtOpRpt.pl) for the given date range
        and court
        """
        ...
        if self._has_security_violation_text(opinions_html):
            raise PacerPageNotAvailableException(
                'Security violation text found instead of opinion page')

        magic_string = self._parse_magic_number_from_opinions_form(
            opinions_html)

        # Then, let's get hte actual report for the dates we are interested in
        req = self._build_opinions_report_req(court, start_date, end_date,
                                              magic_string)
        response = self._get_request(req)
    ...
    def get_pdf_show_doc(self, court, casenum, de_seq_num, dm_id, doc_num):
        ...
        req = self._build_show_doc_req(court, casenum, de_seq_num, dm_id,
                                       doc_num)
        response = self._get_request(req)
        ...
        filename = self._parse_show_temp_filename_from_doc1_page(
            response.read())
    ...
    def _build_opinions_report_req(self, court, start_date, end_date,
                                   magic_string):
    ...
    def _build_show_doc_req(self, court, casenum, de_seq_num, dm_id, doc_num):
        ...
        url += "show_doc.pl?caseid=%s&de_seq_num=%s&dm_id=%s&doc_num=%s" % (
            casenum, de_seq_num,
            dm_id, doc_num
        )
        return urllib2.Request(url)
    ...
    def _is_pdf_response(self, response):
        return response.headers.getheader('content-type').find(
            'application/pdf') >= 0

    def _has_security_violation_text(self, html):
        return html.find(
            'Security violation: You do not have access rights to this '
            'program (WrtOpRpt.pl).') >= 0
